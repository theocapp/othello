...
export default function ForesightPage({ mode, records, error, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: C.bg, zIndex: 200, overflowY: 'auto', animation: 'slideIn 0.3s ease' }}>
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '3rem 2rem 6rem' }}>
        ...
        {error && <div style={{ border: `1px solid ${C.redDeep}`, background: `${C.redDeep}18`, padding: '1rem 1.1rem', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.58rem', color: C.textSecondary, marginBottom: '1rem' }}>{error}</div>}
        {!error && records.length === 0 && <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.6rem', color: C.textSecondary }}>{isPredictions ? 'No briefing-derived predictions have been logged yet.' : 'No early-signal records are available yet.'}</div>}
        {!error && records.map((item, i) => <div key={item.prediction_key || item.event_key || i} style={{ padding: '1rem 0.4rem', borderBottom: `1px solid ${C.border}` }}>
          {isPredictions ? <>
