...
export default function MapSummaryPanel({ data, hotspot, onOpenBriefing, onAnalyzeCluster }) {
  ...
  const panelTopBg = isLightTheme ? C.bgHover : 'rgba(9,11,15,0.68)'
  const contentCardBg = isLightTheme ? C.bgHover : 'rgba(9,12,17,0.56)'
  const rowCardBg = isLightTheme ? C.bgRaised : 'rgba(18,21,27,0.5)'
  const statCardBg = isLightTheme ? C.bgRaised : 'rgba(18,21,27,0.62)'

  return (
    <div style={{ border: `1px solid ${C.border}`, background: `linear-gradient(180deg, ${C.bgRaised}, ${C.bg})`, overflow: 'hidden' }}>
      <div style={{ padding: '0.8rem 1rem', background: panelTopBg, borderBottom: `1px solid ${C.border}`, fontFamily: "'JetBrains Mono', monospace", fontSize: '0.5rem', color: C.textSecondary, letterSpacing: '0.12em', textTransform: 'uppercase' }}>Click near a hotspot centroid to select. Hover to preview. Filter by type using the map legend.</div>
      <div style={{ padding: '1rem 1rem 1.1rem' }}>
        ...
        {hotspot ? <>
          ...
          {leadEvent && (leadTitle || leadBody) && <div style={{ border: `1px solid ${C.borderMid}`, background: contentCardBg, padding: '0.85rem 0.9rem', marginBottom: '0.95rem' }}>
          ...
          {moreEvents.length > 0 && <div style={{ marginBottom: '0.95rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.55rem' }}>{moreEvents.map((ev, idx) => <div key={ev?.event_id || idx} style={{ border: `1px solid ${C.border}`, background: rowCardBg, padding: '0.55rem 0.65rem' }}><div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.42rem', color: C.textMuted, marginBottom: '0.25rem' }}>{ev?.event_date ? formatDateTime(ev.event_date) : '—'}{ev?.fatalities ? ` · ${ev.fatalities} fatalities` : ''}</div><div style={{ fontFamily: "'Source Serif 4', serif", fontSize: '0.8rem', color: C.textSecondary, lineHeight: 1.55 }}>{truncateText(hotspotEventDescription(ev), 180)}</div></div>)}</div>
          </div>}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '0.65rem', marginBottom: '0.95rem' }}>
            {[[isStoryHotspot ? 'Articles' : 'Incidents', hotspot.event_count], [isStoryHotspot ? 'Sources' : 'Fatalities', isStoryHotspot ? hotspot.source_count : hotspot.fatality_total], ['Attention', formatAttentionShare(hotspot.attention_share)], ['Updated', hotspot.latest_event_date ? formatDateTime(hotspot.latest_event_date) : '—']].map(([label, value]) => <div key={label} style={{ border: `1px solid ${C.border}`, padding: '0.7rem 0.75rem', background: statCardBg }}><div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.44rem', color: C.textMuted, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.35rem' }}>{label}</div><div style={{ fontFamily: "'Libre Baskerville', serif", fontSize: label === 'Updated' ? '0.8rem' : '1rem', color: C.textSecondary, lineHeight: 1.35 }}>{value}</div></div>)}
          </div>
          ...
          {onOpenBriefing && <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.15rem' }}>
            ...
            {onAnalyzeCluster && <button onClick={() => onAnalyzeCluster(hotspot, queryTopic)} style={{ width: '100%', background: statCardBg, border: `1px solid ${C.borderMid}`, color: C.textSecondary, fontFamily: "'JetBrains Mono', monospace", fontSize: '0.49rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.65rem 1rem', cursor: 'pointer', textAlign: 'left' }}>Analyze this cluster →</button>}
          </div>}
