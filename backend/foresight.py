...
from corpus import load_before_news_archive, load_event_observation_records, load_prediction_records, search_recent_articles_by_keywords, upsert_event_observations, upsert_prediction_records
...
GENERIC_SUBJECTS = {"monitor", "expect", "articles", "signals", "reporting", "briefing", "sources", "events"}
MAJOR_SOURCES = {
    "reuters",
    "associated press",
...
PREDICTION_TYPE_KEYWORDS = {
    "military": {"strike", "offensive", "escalation", "incursion", "retaliation", "ceasefire", "missile"},
    "diplomacy": {"talks", "negotiation", "summit", "ceasefire", "deal", "agreement", "mediation"},
    "sanctions": {"sanction", "tariff", "restriction", "blacklist", "embargo"},
    "markets": {"rate", "inflation", "market", "recession", "oil", "currency", "yield"},
    "political": {"election", "cabinet", "parliament", "coalition", "protest", "leadership"},
}
CONFIDENCE_MARKERS = {
...
def _hash_key(*parts: str) -> str:
    return hashlib.sha256(" | ".join((part or "").strip() for part in parts).encode("utf-8")).hexdigest()
...
def extract_predictions_from_briefing(topic: str, briefing_text: str, source_ref: str, generated_at: float | None = None, events: list[dict] | None = None) -> list[dict]:
    ...
    for section_name in SECTION_NAMES:
        ...
        for line in _extract_prediction_lines(section):
            horizon_days = _prediction_horizon_days(line, default_days=14 if section_name == "WHAT TO WATCH" else 30)
...
def _prediction_query(record: dict) -> str:
    ...
    if subjects:
        ...
        words = [word for word in re.findall(r"[A-Za-z][A-Za-z-]{3,}", record.get("prediction_text", "")) if word.lower() not in {"likely", "within", "monitor", "expect", "watch", "current", "signals"}]
        query = " ".join(words[:6])
    type_keywords = list(PREDICTION_TYPE_KEYWORDS.get(record.get("prediction_type"), set()))
    if type_keywords:
...
def evaluate_prediction_ledger(topic: str | None = None, limit: int = 200, grace_days: int = 3) -> dict:
    records = load_prediction_records(topic=topic, limit=limit)
    now = time.time()
    updated = []
    counts = {"pending": 0, "resolved_hit": 0, "resolved_miss": 0, "expired_unresolved": 0}

    for record in records:
        if record.get("status") not in {"pending", "expired_unresolved"}:
            counts[record.get("status", "pending")] = counts.get(record.get("status", "pending"), 0) + 1
            continue
        ...
        search_hours = max(72, int(((end_ts - float(record["created_at"])) / 3600) + 24))
        candidate_articles = search_recent_articles_by_keywords(query, topic=record.get("topic"), limit=40, hours=search_hours)
        relevant = [article for article in candidate_articles if _article_within_window(article, float(record["created_at"]), end_ts)]
        unique_sources = sorted({(article.get("source") or "").strip() for article in relevant if article.get("source")})

        if len(unique_sources) >= 2:
            record["status"] = "resolved_hit"
            record["resolved_at"] = now
            record["outcome_summary"] = f"Corroborated by {len(relevant)} articles across {len(unique_sources)} sources inside the prediction window."
            counts["resolved_hit"] += 1
        elif relevant:
            record["status"] = "expired_unresolved"
            record["resolved_at"] = now
            record["outcome_summary"] = "Only weak single-source evidence surfaced inside the prediction window."
            counts["expired_unresolved"] += 1
        else:
            broader_articles = search_recent_articles_by_keywords(" ".join((record.get("extracted_subjects") or [])[:3]) or record.get("prediction_text", ""), topic=record.get("topic"), limit=20, hours=search_hours)
            if broader_articles:
                record["status"] = "resolved_miss"
                record["resolved_at"] = now
                record["outcome_summary"] = "The subject stayed active, but the predicted development did not clearly materialize inside the stated window."
                counts["resolved_miss"] += 1
            else:
                record["status"] = "expired_unresolved"
                record["resolved_at"] = now
                record["outcome_summary"] = "No clear corroborating reporting surfaced before the prediction horizon expired."
                counts["expired_unresolved"] += 1

        payload = dict(record.get("payload") or {})
        payload["evaluation_query"] = query
        payload["matched_urls"] = [article.get("url") for article in relevant if article.get("url")]
...
def observe_events(events: list[dict], observed_at: float | None = None) -> int:
    ...
    for event in events or []:
        ...
        major_articles = [item for item in parsed_articles if (item[1].get("source") or "").strip().lower() in MAJOR_SOURCES]
        first_major_time, first_major_article = major_articles[0] if major_articles else (None, None)
        article_urls = [article.get("url") for _, article in parsed_articles if article.get("url")]
        source_names = sorted({article.get("source") for _, article in parsed_articles if article.get("source")})
        event_key = event.get("event_id") or _hash_key(event.get("topic", ""), event.get("label", ""), "|".join(article_urls[:12]))
        records.append(
            {
                "event_key": event_key,
                ...
                "first_major_source_published_at": first_major_time.isoformat() if first_major_time else None,
                "earliest_source": first_article.get("source"),
                "earliest_major_source": first_major_article.get("source") if first_major_article else None,
...
def load_prediction_ledger(topic: str | None = None, refresh: bool = False, limit: int = 100) -> dict:
    if refresh:
...
def load_early_signal_archive(limit: int = 100, minimum_gap_hours: int = 4) -> dict:
    ...
    if len(archive) < limit:
        ...
        for record in load_event_observation_records(limit=limit * 3):
            ...
            if lead_time_seconds < threshold_seconds or record.get("first_major_source_published_at"):
                continue
            archive.append(
                {
                    **record,
