...
def get_instability_payload(days: int = 3):
    ...
    return cache.get(
        f"cii:{days}", lambda: compute_country_instability(days=days), ttl=TTL_MEDIUM
    )
...
def get_instability_detail_payload(country: str, days: int = 3):
    ...
    for entry in result.get("countries", []):
        if (
            ...
            or (entry.get("label") or "").lower() == normalized
        ):
            return entry
...
def source_reliability_payload(
    ...
    return get_source_reliability(
        topic=topic, days=max(30, min(days, 365)), refresh=refresh
    )


def narrative_drift_payload(
