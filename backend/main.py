...
def root():
    return {
        "status": "Othello V2 API is running",
...
@app.post("/ingest")
def trigger_ingest(topic: str | None = None, _: None = Depends(require_write_access)):
    def run():
        if topic:
            if topic not in TOPICS:
...
def trigger_gdelt_backfill(topic: str | None = None, _: None = Depends(require_write_access)):
    def run():
        if topic:
            if topic not in TOPICS:
                raise HTTPException(status_code=400, detail=f"Topic must be one of {TOPICS}")
...
def source_refresh(_: None = Depends(require_write_access)):
    return {"refresh": refresh_registry_sources(), "mirror": sync_registry_mirror()}


@app.post("/official/refresh")
def official_refresh(_: None = Depends(require_write_access)):
    return refresh_official_updates()


@app.post("/acled/refresh")
def acled_refresh(_: None = Depends(require_write_access)):
    return refresh_acled_events()


@app.post("/gdelt-gkg/refresh")
def gdelt_gkg_refresh(_: None = Depends(require_write_access)):
    return refresh_gdelt_gkg_events()


@app.post("/seed/local")
def trigger_local_seed(_: None = Depends(require_write_access)):
...
def force_refresh(topic: str | None = None, _: None = Depends(require_write_access)):
    if topic:
        if topic not in BRIEFING_TOPICS:
            raise HTTPException(status_code=400, detail=f"Topic must be one of {BRIEFING_TOPICS}")
...
@app.get("/foresight/predictions")
def foresight_predictions(topic: str | None = None, refresh: bool = False, limit: int = 100):
    return load_prediction_ledger(topic=topic, refresh=refresh, limit=max(1, min(limit, 300)))


@app.get("/foresight/before-news")
...
def query(request: QueryRequest):
    ...
    if REQUEST_ENABLE_LLM_RESPONSES and os.getenv("GROQ_API_KEY"):
        try:
            ...
            print(f"[query] LLM analysis failed, using corpus fallback: {exc}")
...
def get_timeline(query: str):
    ...
    try:
        ...
        print(f"[timeline] Timeline build failed for '{meta['focus']}', using corpus fallback: {exc}")
        return _timeline_fallback(query, articles)
...
@app.get("/entities/reference/{entity}")
...
@app.get("/narratives/drift/{subject}")
